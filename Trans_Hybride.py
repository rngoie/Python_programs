...
def  minimumL(L,C,M,Min,taille1,taille2):
    for i in range(0,taille1):
        mini=10000000000
        for j in range(0,taille2):
            if mini>M[i][j]:
...
def  minimumC(L,C,M,Min,taille1,taille2):
    for i in range(0,taille1):
        mini=1000000000000
        for j in range(0,taille2):
            if mini>M[j][i]:
...
def reductionL(L,C,M,MinLC,taille1,taille2):
    for i in range(0,taille1):
        for j in range(0,taille2):
            if L[i]==0 and C[j]==0:
                M[i][j]=M[i][j]-MinLC[i]
...
def reductionC(L,C,M,MinLC,taille1,taille2):
...
def Hybride(Mtrx,Dispox,Demandx,n,m):   
    absc=0
    ordn=0
    cout=0
    nbl=n
    ...
    MinL=np.zeros((n),dtype="f")
    MinC=np.zeros((m),dtype="f")
...
#debut algo
    ...
    while nbl>=2 and nbc>=2:
         minimumL(BarL,BarCol,Mtrx,MinL,n,m)
    #REDUCTION ligne
         reductionL(BarL,BarCol,Mtrx,MinL,n,m)
    #MINIMUM
    
         minimumC(BarL,BarCol,Mtrx,MinC,m,n)
    #reduction colonne
    
         reductionC(BarL,BarCol,Mtrx,MinC,m,n)
...
print(Hybride([[14., 64., 28., 88., 37., 22., 21.],  [31., 18., 71., 55., 54., 26., 84.],  [ 6., 10., 16., 82., 43.,  9., 61.]], [91., 90., 80.], [ 33.,  18.,  33.,  22.,  17.,  22., 116.], 3, 7))
